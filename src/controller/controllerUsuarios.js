...
const getById = async (req, res) => {
    try {
        const usuariosId = await Usuarios.findById(req.params.id)
...
const substituirUsuario = async (req, res) => {
    try {
        const usuariosEncontrados = await Usuarios.where().findOneAndReplace({ _id: req.params.id }, req.body, { new: true })
        res.status(200).send(usuariosEncontrados)
    }
    catch (error) {
        res.status(500).send({ message: error.message })
